...
node --input-type=module -e "import { AccessDeniedError, hasDefaultPluginAccess, MC_AVAILABILITY, MC_CONDITION } from 'payload-plugin-gmc-ecommerce/v2'; const denied = new AccessDeniedError(); if (denied.statusCode !== 403 || !MC_AVAILABILITY.includes('IN_STOCK') || !MC_CONDITION.includes('NEW') || hasDefaultPluginAccess({ user: { isAdmin: true } }) !== true || hasDefaultPluginAccess({ user: null }) !== false) { throw new Error('v2 access and vocabulary exports are incomplete') }"
