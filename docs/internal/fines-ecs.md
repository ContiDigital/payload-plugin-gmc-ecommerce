...
3. [ ] Generate, review, apply, and rollback-test the owner-created Payload migration for lineage/immutable fields and indexes plus the complete v2 publication-state contract: unique indexed `key`; indexed `productId`, `status`, and `storeCode`; unindexed identity/scope/operation/revision columns; `desiredAt` and `desiredDigest`; published/observed timestamps; remote status/missing/version; structured error; timestamps enabled and versions disabled. Verify `(root_operation_id,status)`, `(parent_operation_id,status)`, and `(type,raw_subject,status)` plus the retained unique `immutable_key`. Retire v1 shadow schema only in the later cleanup migration after rollback retention.
